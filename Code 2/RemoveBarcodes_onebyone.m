function [OverallMCC,ProbesRemoved,Opt_Thresh_probes,Good_probes_final,R_Cond_Barcodes_final,ProbesRemoved_names] = RemoveBarcodes_onebyone(Good_probes,R_Cond_final,noise,thresh,names)
...
OverallMCC = zeros(length(Good_probes),1);
    for i=1:length(Good_probes)
        [fhatmat_barcodes,fmat_barcodes]=f_and_fhat_barcodes_hybrid(Good_probes,R_Cond_final,noise,3);
 %       [fhatmat_barcodes,fmat_barcodes]=f_and_fhat_barcodes_hybrid_rnd_conc(Good_probes,R_Cond_final,noise,3);
