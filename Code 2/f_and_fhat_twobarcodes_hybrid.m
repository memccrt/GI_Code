function [fhatmat,fmat]=f_and_fhat_twobarcodes_hybrid(probes,R,noise,ntrials)
...
for i=1:n
    ...
    rng('shuffle');
        ...
        f(j)=1;
        if j<= N_BATCH_A
            f(randi([1 N_BATCH_A],1,1))=1;
            f(randi([N_BATCH_A+1 N_PROBES],1,2))=1;
        else
            f(randi([N_BATCH_A+1 N_PROBES],1,1))=1;
            f(randi([1 N_BATCH_A],1,2))=1;
        end
